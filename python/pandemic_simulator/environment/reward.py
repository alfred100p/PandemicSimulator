...
class RewardFunction(metaclass=ABCMeta):
    ...
    @abstractmethod
    def calculate_reward(self, prev_obs: np.array, action: int, obs: np.array, unlocked_non_essential_business_locations:np.array) -> float:
        pass
...
class SumReward(RewardFunction):
    ...
    def calculate_reward(self, prev_obs: np.array, action: int, obs: np.array, unlocked_non_essential_business_locations:np.array) -> float:
        rewards = np.array([rf.calculate_reward(self, prev_obs, action, obs, unlocked_non_essential_business_locations) for rf in self._reward_fns])
...
class InfectionSummaryAboveThresholdReward(RewardFunction):
    ...
    def calculate_reward(self, prev_obs: np.array, action: int, obs: np.array, unlocked_non_essential_business_locations:np.array) -> float:
        return float(-1 * max(np.mean(obs[:5]
...
class SmoothStageChangesReward(RewardFunction):
    ...
    def calculate_reward(self, prev_obs: np.array, action: int, obs: np.array, unlocked_non_essential_business_locations:np.array) -> float:
        return float(-1 * np.abs(obs[10] - prev_obs[10]).mean())
